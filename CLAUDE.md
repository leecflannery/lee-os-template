...
<!-- Hooks are shell scripts that run automatically before or after Claude uses a tool.
     ...
     Configuration lives in .claude/settings.json. Two included hooks ship with this template:
...
2. **NEVER take actions visible to others without confirmation.** Calendar events with attendees, shared documents, external posts — always ask first.
